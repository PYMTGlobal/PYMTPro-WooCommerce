...
class PYMTPro
{
    const API_BASE = 'https://api.pymtpro.com/v1/';
    private $_rpc;
    private $_authentication;


    public static function withApiToken($key, $secret)
    {
        return new PYMTPro(new PYMTPro_ApiKeyAuthentication($key, $secret));
    }

    public static function withSimpleApiToken($key)
    {
        return new PYMTPro(new PYMTPro_SimpleApiKeyAuthentication($key));
    }

    public static function withOAuth($oauth, $tokens)
    {
        return new PYMTPro(new PYMTPro_OAuthAuthentication($oauth, $tokens));
    }

    // This constructor is deprecated.
    public function __construct($authentication, $apiKeySecret=null, $tokens=null)
    {
        // First off, check for a legit authentication class type
        if (is_a($authentication, 'PYMTPro_Authentication')) {
            ...
            if ($tokens !== null) {
                $this->_authentication = new PYMTPro_OAuthAuthentication($authentication, $tokens);
            } else if ($authentication !== null && is_string($authentication)) {
                $apiKey = $authentication;
                if ($apiKeySecret === null) {
                    // Simple API key
                    $this->_authentication = new PYMTPro_SimpleApiKeyAuthentication($apiKey);
                } else {
                    $this->_authentication = new PYMTPro_ApiKeyAuthentication($apiKey, $apiKeySecret);
                }
            } else {
                throw new PYMTPro_ApiException('Could not determine API authentication scheme');
            }
        }

        $this->_rpc = new PYMTPro_Rpc(new PYMTPro_Requestor(), $this->_authentication);
    }

    // Used for unit testing only
    public function setRequestor($requestor)
    {
        $this->_rpc = new PYMTPro_Rpc($requestor, $this->_authentication);
        return $this;
    }
    ...
    public function createButtonWithOptions($options=array())
    {
        ...
        $response = $this->post("order/new", array( "button" => $options ));
        ...
        $returnValue->embedHtml = "<div class=\"PYMTPro-button\" data-code=\"" . $response->button->code . "\"></div><script src=\"https://PYMTPro.com/assets/button.js\" type=\"text/javascript\"></script>";
